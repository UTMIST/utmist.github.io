...
async function grabEvents() {
    ...
    eventsJSON.forEach(event => {
        const maybeid = event["Cover Photo"].replace("https://drive.google.com/open?id=", "");
        ...
        if (pic)
            ...
            console.error("no pic found for " + maybeid);
    });
...
async function grabEventPics() {
    ...
    return Promise.all(fileMetas.map(f => {
        ...
        return (drive.files
            ...
            .then(res => {
            const data = res.data;
            data.pipe(fs_1.default.createWriteStream("./files/" + name));
            return { id, name };
        }));
    }));
}
